...
def write_motivation_section(page):
    ...
    col1.markdown('<p class="custom-percentage">74% of the files discussed two to four categories of information</p>', unsafe_allow_html=True)
    col2.markdown('<p class="custom-percentage">15% of the files presented only one category of information</p>', unsafe_allow_html=True)
    ...
    col1, col2, col3 = page.columns(3)
    col1.markdown('<p class="custom-percentage">74% of the files included two to four categories of information</p>', unsafe_allow_html=True)
    col2.markdown('<p class="custom-percentage">15% of the files presented only one category of information</p>', unsafe_allow_html=True)
    col3.markdown('<p class="custom-percentage">On the other hand, only 10% of the files included more than five categories</p>', unsafe_allow_html=True)
    page.markdown("Based on these findings, the researchers implemented a classification model using the data obtained from the qualitative analysis. This model was implemented to investigate a bigger sample of projects without the need of a manual annotation of CONTRIBUTING.md files, and also to built a tool capable of reporting the quality of such files to project maintainers. Our future goal is to support newcomers and maintainers on the implementation of better documentation files.")
